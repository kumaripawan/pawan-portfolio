...
export default function Projects() {
  return (
    <section id="projects" className="container mx-auto px-4 py-10 scroll-mt-28">
      <h2 className="text-3xl font-semibold text-center">
        <span className="bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-pink-600 bg-clip-text text-transparent">
          Projects
        </span>
      </h2>
      <p className="text-center text-white-400 mt-2">A selection of recent work.</p>
        ...
        {projects.map((p) => (
          <a
            ...
            className="group relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm
                       ...
                       hover:-translate-y-1 hover:bg-white/10 hover:shadow-lg focus:outline-none
          ...
          >
            ...
            {Array.isArray(p.tags) && p.tags.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded-full border border-white/15 bg-white/10 px-2 py-0.5
                               text-xs text-gray-200 transition group-hover:-translate-y-0.5 group-hover:shadow-sm"
                  >
                    {t}
                  </span>
                ))}
            ...
            <span
              className="mt-4 inline-flex items-center text-sm text-indigo-300 underline
                         decoration-indigo-500/40 underline-offset-4 hover:text-indigo-200
